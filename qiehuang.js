/**
 ...
 7-13 新增好友协助，优化偷好友阳光。
 7-15 UA随机获取。
 9-19 修复数据，去掉了脚本检测更新功能 &${sign()}。来自 lksky8
 10-13 新增删除等级低于4级的好友，系统将在日期为5的倍数的时候删除 等级低于4级的好友
 */
 ...
 const uaNum = Math.round(Math.random() * 22); //随机UA，从0-21随便选一个填上去
 //////////////////////
 let tyau = "";
  ...
  * 偷朋友阳光
  */
 async function stealFriendSunshine(timeout = 2) {
   //--0 先获取好友列表
   let url = {
     url: `http://api.xiaoyisz.com/qiehuang/ga/user/friend/list?page=1&size=50&${sign()}`,
     headers: {
       Host: "api.xiaoyisz.com",
       authorization: `${tyau}`,
       "user-agent": `${ua}`,
         ...
         try {
           ...
           if (result.code == 904) {
             ...
             if (content.length > 0) {
               ...
               const helpList = content.filter((it) => it.helpAble);
               const delList = content.filter((it) => it.level < 4);
               //排个序，优先协助等级高的
               helpList.sort((a, b) => b.level - a.level);
 
               log(`可偷取好友有${stealList.length}个`);
               log(`可协助好友有${helpList.length}个`);
               let stealNums = 0;
               //--2 开始偷，然后汇总偷取的阳光
               if (stealList.length > 0) {
                 for (let i = 0; i < stealList.length; i++) {
                   const crt = stealList[i];
                   if (stealFull) {
                     log("超过偷取人数，不再偷取");
                     break;
                   } else {
                     const stealNum = await stealSunshine(crt.userId);
                     await $.wait(2);
                     stealNums += stealNum;
                   }
                 }
                 log(`共偷取了阳光 ${stealNums}g`);
               }
               //--3 开始协助
               if (helpList.length > 0) {
                 for (let i = 0; i < helpList.length; i++) {
                   const crt = helpList[i];
                   if (helpFull) {
                     log("超过协助人数，今天不再协助");
                     break;
                   } else {
                     await helpFriend(crt.userId);
                     await $.wait(2);
                   }
                 }
               }
               ...
               // 删除好友
               log("系统将在日期为5的倍数的时候删除 等级低于4级的好友");
               const today = new Date().getDay();
               if (today % 5 == 0 && delList.length > 0) {
                 log(`准备删除${delList.length}个等级低于4级的好友`);
                 for (let i = 0; i < delList.length; i++) {
                   const crt = delList[i];
                   await delFriend(crt.userId);
                   await $.wait(2);
                 }
               }
  ...
  * 删除等级低于4级的好友
  */
 function delFriend(userId, timeout = 2) {
   return new Promise((resolve) => {
     let url = {
       url: `http://api.xiaoyisz.com/qiehuang/ga/user/friend/del?${sign()}&friendUserId=${userId}`,
       headers: {
         Host: "api.xiaoyisz.com",
         authorization: `${tyau}`,
         "user-agent": `${ua}`,
  ...
  * 偷好友阳光
  */
 function stealSunshine(userId, timeout = 2) {
   return new Promise((resolve) => {
     let url = {
       url: `http://api.xiaoyisz.com/qiehuang/ga/user/daily/steal?${sign()}&friendUserId=${userId}`,
       headers: {
         Host: "api.xiaoyisz.com",
         authorization: `${tyau}`,
         "user-agent": `${ua}`,
  ...
  * 协助培育
  */
 async function helpFriend(userId, timeout = 2) {
   const plantId = await getPlantFriend(userId);
   return new Promise((resolve) => {
     let url = {
       url: `https://api.xiaoyisz.com/qiehuang/ga/plant/giveSunshine?plantId=${plantId}&${sign()}`,
       headers: {
         Host: "api.xiaoyisz.com",
         authorization: `${tyau}`,
         "user-agent": `${ua}`,
 ...
 function report(num) {
   let url = {
     url:
       `http://api.xiaoyisz.com/qiehuang/ga/user/task/report?` +
       sign() +
       `&taskType=${taskTypeArr[num]}&attachId=${timestampMs()}&taskId=${
         taskIdArr[num]
       }`,
     headers: {
       Host: "api.xiaoyisz.com",
 ...
 function getDrawPriz(num) {
   let url = {
     url:
       `http://api.xiaoyisz.com/qiehuang/ga/user/task/drawPrize?` +
       sign() +
       `&taskId=${taskIdArr[num]}`,
     headers: {
       Host: "api.xiaoyisz.com",
       authorization: `${tyau}`,
       "user-agent": `${ua}`,
  ...
  * 获取植物朋友家植物详情
  */
 function getPlantFriend(userId) {
   let url = {
     url: `http://api.xiaoyisz.com/qiehuang/ga/plant/info?userId=${userId}&${sign()}`,
     headers: {
       Host: "api.xiaoyisz.com",
       authorization: `${tyau}`,
       "user-agent": `${ua}`,
  ...
  * 开始挑战
  */
 function startCallenge(timeout = 2) {
   let url = {
  ...
  * 上报挑战
  */
 function reportCallenge(timeout = 2) {
   let url = {
  ...
  * 开始冒险
  */
 function startAdventure(timeout = 2) {
   let url = {
  ...
  * 查询冒险
  */
 function queryAdventure(timeout = 2) {
   let url = {
     url:
       "http://api.xiaoyisz.com/qiehuang/ga/user/adventure/info?" +
       sign() +
       "&userId=-1&type=2",
     headers: {
       Host: "api.xiaoyisz.com",
  ...
  * 上报冒险
  */
 function reportAdventure(timeout = 2) {
   let url = {
     url:
       "http://api.xiaoyisz.com/qiehuang/ga/user/adventure/drawPrize?" +
       sign() +
  ...
  * 洒阳光
  */
 function giveSunshine(timeout = 2) {
   let url = {
     url:
       "http://api.xiaoyisz.com/qiehuang/ga/plant/batchgiveSunshine?" +
       sign() +
       `&plantId=${tyPlantId}`,
     headers: {
       Host: "api.xiaoyisz.com",
  ...
  * 浇水
  */
 function upgrade(timeout = 2) {
   let url = {
     url:
       "http://api.xiaoyisz.com/qiehuang/ga/plant/upgrade?" +
       sign() +
       `&plantId=${tyPlantId}`,
     headers: {
       Host: "api.xiaoyisz.com",
  ...
  * 收取阳光
  */
 function getSunshine(timeout = 2) {
   let url = {
  ...
  * 收植物
  */
 function getHarvest(timeout = 2) {
   let url = {
     url:
       "http://api.xiaoyisz.com/qiehuang/ga/plant/harvest?" +
       sign() +
       `&plantId=${tyPlantId}`,
     headers: {
       Host: "api.xiaoyisz.com",
  ...
  * 种植物
  */
 function getNewPlant(timeout = 2) {
   let url = {
  ...
  * 获取信息
  */
 function getUserInfo(timeout = 2) {
   let url = {
     url: `http://api.xiaoyisz.com/qiehuang/ga/user/info?` + sign(),
     headers: {
       Host: "api.xiaoyisz.com",
 ...
 function sign() {
   ...
   return (
     "timestamp=" +
     t +
     "&nonce=6M43dZciwRpSiFDY&signature=" +
     cryptoPassFunc(
       "clientKey=IfWu0xwXlWgqkIC7DWn20qpo6a30hXX6&clientSecret=A4rHhUJfMjw2I5CODh5g40Ja1d3Yk1CH&nonce=6M43dZciwRpSiFDY&timestamp=" +
         t
     )
   );
 }
